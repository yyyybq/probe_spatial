"""Per-task evaluation on val set using best checkpoints."""
import sys, os, torch, json
sys.path.insert(0, os.path.dirname(__file__))

from vidfm3d.models.components.probe_pixalign import ProbeModelPA
from vidfm3d.models.video_probe_module import VideoProbeLitModule
from vidfm3d.data.components.inscene15k_dataset import InsScene15KDataset
from vidfm3d.utils.loss import camera_loss, depth_loss, identity_loss, point_loss

DEVICE = torch.device(f"cuda:{sys.argv[1]}" if len(sys.argv) > 1 else "cuda:7")

CONFIGS = {
    "wan": {
        "ckpt": "logs/inscene15k/runs/inscene15k_wan_probe/checkpoints/epoch_029.ckpt",
        "video_channels": 1536,
        "vfm_name": "wan",
    ...
    "cogvideox": {
        "ckpt": "logs/inscene15k/runs/inscene15k_cogvideox_probe/checkpoints/epoch_035.ckpt",
        "video_channels": 3072,
        "vfm_name": "cogvideox",
        "feat_postfix": "_t749_layer20",
        "batch_size": 2,
    ...
    "vjepa2": {
        "ckpt": "logs/inscene15k/runs/inscene15k_vjepa2_probe/checkpoints/epoch_031.ckpt",
        "video_channels": 1024,
        "vfm_name": "vjepa2",
...
def load_model(cfg):
    probe = ProbeModelPA(
        ...
        active_heads=["depth", "point", "camera", "identity"],
        identity_dim=64,
    )
...
def evaluate(model_name, cfg):
    ...
    for i, batch in enumerate(loader):
        ...
        if "world_points" in preds:
            output["pmaps"] = preds["world_points"].permute(0, 1, 4, 2, 3)
        if "depth" in preds:
        ...
        if "pmaps" in output:
            pmap_dict = point_loss(
                output["pmaps"].float().permute(0, 1, 3, 4, 2),
                conf_map.float(),
                batch["pmaps"].float().permute(0, 1, 3, 4, 2),
                mask=fg_mask,
                normalize_pred=True,
                normalize_gt=True,
                gradient_loss="grad",
            )
            losses["point"] = pmap_dict["loss_point"].item()
        ...
        total = sum(losses.get(k, 0) for k in ["point", "depth", "camera", "identity"])
        losses["total"] = total
...
if __name__ == "__main__":
    ...
    print(f"{'='*60}")
    keys = ["point", "depth", "camera", "identity", "total", "Auc_3", "Auc_5", "Auc_10", "Auc_30", "Rac_5", "Rac_15", "Tac_5", "Tac_15"]
    header = f"{'Metric':15s} {'WAN':>10s} {'CogVideoX':>10s} {'V-JEPA2':>10s}"
    print(header)
    print("-" * len(header))
    for k in keys:
        vals = [all_results[m].get(k, float('nan')) for m in ["wan", "cogvideox", "vjepa2"]]
        best = min(vals) if k in ["point", "depth", "camera", "identity", "total"] else max(vals)
        row = f"{k:15s}"
        for v in vals:
