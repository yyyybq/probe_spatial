    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.identity_head = (
            DPTHead(
                dim_in=embed_dim * 2,
                output_dim=kwargs.get("identity_dim", 64),
                activation="linear",
                conf_activation="expp1",
                features=dpt_dim,
                out_channels=dpt_stage_channels,
    ...
    def forward(
        ...
        with torch.amp.autocast("cuda", enabled=False):
            ...
            if self.identity_head is not None:
                identity_feat, identity_conf = self.identity_head(
                    aggregated_tokens_list,
                    n_patches=(video_features.shape[3], video_features.shape[4]),
                    frames_shape=video_shape,
                    patch_start_idx=patch_start_idx,
                )
                predictions["identity"] = identity_feat
                predictions["identity_conf"] = identity_conf
