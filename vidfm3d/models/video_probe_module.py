...
class VideoProbeLitModule(LightningModule):
    ...
    def forward(self, data: Dict[str, torch.Tensor]) -> Any:
        ...
        if "identity" in output:
            output_dict["identity"] = output["identity"]  # (B, S, H, W, D)
            output_dict["identity_conf"] = output["identity_conf"]  # (B, S, H, W)
    ...
    def model_step(
        ...
        if "identity" in preds and self.loss_weights.get("identity", 0) > 0:
            id_loss_dict = identity_loss(
                preds["identity"],  # (B, S, H, W, D)
                batch["identity_ids"],  # (B, S, H, W)
                mask=fg_mask,
            )
            loss += self.loss_weights["identity"] * id_loss_dict["loss_identity"]
            loss_details.update(id_loss_dict)
