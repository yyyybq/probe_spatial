...
def identity_loss(pred_embed, gt_ids, mask=None):
    """
    Discriminative embedding loss for instance identity segmentation.

    For each scene, we compute a per-pixel cross-entropy loss where each unique
    instance ID in the ground truth is treated as a class. The predicted embedding
    is projected to class logits via dot product with learned-free per-instance
    mean embeddings (prototypes).

    Args:
        pred_embed: (B, S, H, W, D) predicted per-pixel embeddings
        gt_ids: (B, S, H, W) ground truth instance IDs (integer)
        mask: (B, S, H, W) optional valid pixel mask

    Returns:
        dict with loss_identity
    """
    B, S, H, W, D = pred_embed.shape
    device = pred_embed.device

    if mask is None:
        mask = torch.ones(B, S, H, W, device=device, dtype=torch.bool)
    else:
        mask = mask > 0
    ...
    for b in range(B):
        # Flatten across frames for this scene
        embed_b = pred_embed[b].reshape(-1, D)  # (S*H*W, D)
        ids_b = gt_ids[b].reshape(-1)  # (S*H*W,)
        mask_b = mask[b].reshape(-1)  # (S*H*W,)

        # Filter valid pixels
        valid = mask_b & (ids_b >= 0)
        if valid.sum() < 10:
            continue

        embed_valid = embed_b[valid]  # (N, D)
        ids_valid = ids_b[valid]  # (N,)

        # Remap instance IDs to contiguous class indices
        unique_ids = ids_valid.unique()
        num_classes = len(unique_ids)
        if num_classes < 2:
            continue

        id_to_idx = torch.zeros(unique_ids.max() + 1, dtype=torch.long, device=device)
        id_to_idx[unique_ids] = torch.arange(num_classes, device=device)
        class_indices = id_to_idx[ids_valid]  # (N,)

        # Compute prototype (mean embedding) for each class
        prototypes = torch.zeros(num_classes, D, device=device)
        proto_count = torch.zeros(num_classes, 1, device=device)
        prototypes.scatter_add_(0, class_indices.unsqueeze(1).expand(-1, D), embed_valid)
        proto_count.scatter_add_(
            0, class_indices.unsqueeze(1), torch.ones(len(ids_valid), 1, device=device)
        )
        prototypes = prototypes / proto_count.clamp(min=1)

        # Compute logits: dot product of each pixel embedding with all prototypes
        logits = embed_valid @ prototypes.t()  # (N, num_classes)

        loss_b = F.cross_entropy(logits, class_indices)
        loss_b = check_and_fix_inf_nan(loss_b, "identity_loss")
    ...
    return {"loss_identity": total_loss}
